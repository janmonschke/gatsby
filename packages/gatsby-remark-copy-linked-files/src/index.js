...
module.exports = (
  ...
  visit(markdownAST, `html`, node => {
    ...
    // extracts all elements that have the provided attribute
    function extractUrlAttribute(selection, attribute) {
      return (
        selection
          ...
          .toArray()
          // don't take undefined values into account
          .filter(Boolean)
      )
    }
    ...
    // Handle Images
    extractUrlAttribute($(`img`), `src`).forEach(({ url, element }) => {
      try {
        const ext = url.split(`.`).pop()
        if (!options.ignoreFileExtensions.includes(ext)) {
    ...
    // Handle video tags.
    extractUrlAttribute($(`video source, video`), `src`).forEach(processUrl)

    // Handle audio tags.
    extractUrlAttribute($(`audio source`), `src`).forEach(processUrl)

    // Handle a tags.
    extractUrlAttribute($(`a`), `href`).forEach(processUrl)
